...
export const Sidebar = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed md:static inset-y-0 left-0 z-40 w-64 bg-slate-900 border-r border-slate-800 flex flex-col transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="p-5 border-b border-slate-800 flex items-center justify-between">
          <Link to={getDashboardPath()} className="flex items-center gap-3" onClick={onClose}>
            ...
            <div>
              <h1 className="font-extrabold text-slate-100 text-base leading-tight">
                Coaching<span className="text-brand-500">.ai</span>
              </h1>
              <p className="text-[10px] text-slate-400 font-medium">Smart Student Platform</p>
            </div>
          </Link>
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-xl text-slate-400 hover:bg-slate-800 md:hidden transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-all ${
                    isActive
                      ...
                      : item.highlight
                      ? 'bg-gradient-to-r from-amber-500/10 to-brand-500/10 text-amber-400 border border-amber-500/20 hover:bg-amber-500/20'
        ...
        {user && (
          <div className="p-4 border-t border-slate-800">
            <Link to="/profile" onClick={onClose} className="block">
              <div className="bg-slate-800/60 border border-slate-700/60 rounded-xl p-3 flex items-center gap-3 hover:bg-slate-800 transition-colors">
                <img
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-amber-400 font-medium">⚡ {user.reputation || 0} pts</p>
                </div>
                <span className="text-[10px] bg-slate-700 text-slate-200 px-2 py-0.5 rounded-full font-bold shrink-0">
                  {user.level || 'Beginner'}
                </span>
              </div>
            </Link>
